...
namespace spatial
{
  namespace details
  {
     ...
     *  It will not work for user defined comparators; it means that if you are
     *  using a user-defined comparator in your container, you should also use
     ...
     */
    //@{
    template<typename Compare, typename DistanceType>
    struct builtin_difference_cast { };

    template<typename Compare, typename DistanceType>
    struct builtin_difference_cast<bracket_less<Key>, DistanceType>
    {
      typedef bracket_minus<Key, DistanceType> type;
      type operator() (const bracket_less<Key>&) const
      { return type(); }
    };

    template<typename Key, typename DistanceType>
    struct builtin_difference_cast<paren_less<Key>, DistanceType>
    {
      typedef paren_minus<Key, DistanceType> type;
      type operator() (const paren_less<Key>&) const
      { return type(); }
    };

    template<typename Key, typename DistanceType>
    struct builtin_difference_cast<iterator_less<Key>, DistanceType>
    {
      typedef iterator_minus<Key, DistanceType> type;
      type operator() (const iterator_less<Key>&) const
      { return type(); }
    };

    template<typename Key, typename DistanceType, typename Accessor>
    struct builtin_difference_cast<accessor_less<Accessor, Key>, DistanceType>
    {
      typedef accessor_minus<Accessor, Key, DistanceType> type;
    ...
    struct with_builtin_difference
    <Container, DistanceType, enable_if<is_compare_builtin<Container> >::type>              
    {
      typedef typename builtin_difference_cast
        <typename container_trait<Container>::key_compare, DistanceType>::type
        type;
      type operator()(const Container& container) const
      {
        return builtin_difference_cast
          <container_trait<Container>::compare_type, DistanceType>()
          (container.key_comp());
      }
    };
    //@}
