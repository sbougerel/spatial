  ...
  namespace details
  {
	...
	(const Rank& rank, const Compare& cmp, const Geometry& geometry,
	 ...
	 const distance_type& distance = distance_type())
	  : Rank(rank), origin(cmp, origin), distance(geometry, distance),
	    node_dim(node_dim), node(node)
	{ }

	/**
	 *  @brief  The origin point from which all distances are calculated
	 *
	 ...
	 */
	details::Compress<Compare, Key> origin;
	 ...
	 */
	details::Compress<Geometry, distance_type> distance;
	...
	Base_ptr node;
      ...
      const Compare&
      compare() const
      { return impl.origin.base(); }

      const Geometry&
      geometry() const
      { return impl.distance.base(); }
    ...
    public:
       ...
       *  @param origin The point of origin of the search.
      ...
      lower_bound(const Rank& rank, const Compare& cmp, const Geometry& geometry,
		  const Key& origin, distance_type lower_limit,
      ...
      upper_bound(const Rank& rank, const Compare& cmp, const Geometry& geometry,
		  const Key& origin, distance_type upper_limit,
		  dimension_type node_dim, Base_ptr node);
