...
namespace spatial
{
  namespace details
  {
    ...
    (const key_type& key, const Predicate& predicate)
    {
      ...
      for (equal_iterator iter = found.first; iter != found.second; ++iter)
        {
          if (predicate(*iter, key))
            return iterator(static_cast<Link_type>(iter.impl_.node_));
        }
      return end();
    }
    ...
    (const key_type& value, const Predicate& predicate) const
    {
      ...
      std::pair<const_equal_iterator, const_equal_iterator> found
        = equal_range(value);
      for (const_equal_iterator iter = found.first;
           iter != found.second; ++iter)
        {
